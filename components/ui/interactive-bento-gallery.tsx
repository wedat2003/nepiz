...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType, className?: string, onClick?: () => void }) => {
    ...
    if (item.type === 'video') {
        return (
            <div className={`${className} relative overflow-hidden bg-black/30`}>
                <video
                    ref={videoRef}
    ...
    return (
        <>
            ...
            >
                ...
                <div className="absolute right-2 top-2 z-20 sm:right-2.5 sm:top-2.5 md:right-3 md:top-3">
                    <motion.button
                        className="rounded-full bg-gray-200/80 p-2 text-gray-700 hover:bg-gray-300/80 text-xs sm:text-sm backdrop-blur-sm"
                        onClick={onClose}
...
interface InteractiveBentoGalleryProps {
    ...
    onToggleSelect?: (item: MediaItemType) => void
}
        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="mx-auto grid max-w-6xl grid-cols-1 gap-4 auto-rows-[120px] sm:grid-cols-2 sm:auto-rows-[120px] md:grid-cols-4 md:auto-rows-[135px]"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
                    ...
                    >
                        {items.map((item, index) => (
                            ...
                            >
                                ...
                                />
                                {selectionMode ? (
                                    <div className={`absolute right-3 top-3 h-6 w-6 rounded-full border-2 ${
                                        selectedIds.includes(item.id)
                                            ? 'border-fuchsia-300 bg-fuchsia-400/70'
